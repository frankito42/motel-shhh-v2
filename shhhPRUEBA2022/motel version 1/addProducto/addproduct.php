...
$sqlTodosLosArticulos="SELECT `articulo`, `nombre`,
 `tipoart`, `costo`, `stockmin`,
  `cantidad`, `descripcion`, `imagen`,
   `categoria`, `codBarra`, `precioVenta`,
    `mostrar`, `fechaVencimiento`,datediff(`fechaVencimiento`,NOW()) as diasFaltantes FROM `articulos`";
			...
			<table id="mytable" class="table table-hover " style="margin-top:20px;">
				...
				<tbody>
					<?php
    					...
    					$db = $database->open();
						try{	
						    $sql = 'SELECT `articulo`, `nombre`,
                `tipoart`, `costo`, `stockmin`,
                 `cantidad`, `descripcion`, `imagen`,
                  `categoria`, `codBarra`, `precioVenta`,
                   `mostrar`, `fechaVencimiento`,datediff(`fechaVencimiento`,NOW()) as diasFaltantes FROM `articulos` ORDER BY `categoria`';
						    foreach ($db->query($sql) as $row) {
                    if ($row['diasFaltantes']<60 && !empty($row['diasFaltantes'])) {
                      $style="background: #dc354582;";
                    }else{
                      $style="";
                    }
						    	...
						    	<tr style="<?php echo $style;?>">
						    		...
						    		<td><?php echo $row['diasFaltantes']." dias"; ?></td>
						    		<td>
                      <?php
                      if ($row["imagen"]=="") {
                        echo "no hay img";
