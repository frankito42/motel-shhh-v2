...
if (isset($_GET['fin'])) {
    ...
    /* aqui se pregunta se ya si inserto en caja para no generar una copia */
    $sqlCajaPregunta="SELECT * FROM `cajas` WHERE `idMovtemp`=:idM";
    $resCajaPregunta=$conexion->prepare($sqlCajaPregunta);
    $resCajaPregunta->bindParam(":idM",$id_movtem);
    $resCajaPregunta->execute();
    $resCajaPregunta=$resCajaPregunta->fetch(PDO::FETCH_ASSOC);

    if ($resCajaPregunta==null) {
