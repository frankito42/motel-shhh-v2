      ...
      /*   $(aa).mousedown(function(e){
            //1: izquierda, 2: medio/ruleta, 3: derecho
             if(e.which == 3){
                ...
                localStorage.setItem("estadoHabitacion",estadoHabitacion)
                 ...
                 if(estadoHabitacion=="ocupada"||estadoHabitacion=="cobrando"){
                    ...
                    .then((data) =>{ 
                        ...
                        <div style="background: #dee2e6;" class="modal-content">
                            ...
                            <a type="button" style="background: #ba9d0f !important;color: #ffffff;" class="btn btn-danger" onclick="activarAlarmaHabit()"><i class="fas fa-bell"></i></a>
                            </div>
                        </div>
                        <!--/.Content-->
                        </div>
...
async function activarAlarmaHabit(){



    if(document.getElementById("modalAlarma"+localStorage.getItem("idHabitacion"))){
        document.getElementById("modalAlarma"+localStorage.getItem("idHabitacion")).remove()
    }
    let modalClickDerecho=`<div class="modal fade" id="modalAlarma${localStorage.getItem("idHabitacion")}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-notify modal-success" role="document">
    <!--Content-->
    <div style="background: #dee2e6;" class="modal-content">
        <!--Header-->
        <div style="background:#a30000fc;" class="modal-header">
        <p style="color: #adb4b4e6;font-size: 180%;" class="heading lead">Habitacion N°${localStorage.getItem("habitacion")}</p>
        ...
        <input id="tiempoAlarma" class="form-control" type="datetime-local">
        ...
        <div class="modal-footer">
        <a type="button" data-dismiss="modal" style="background: #a40303 !important;color: #aca2a2;" class="btn btn-danger">Cerrar</a>
        <a type="button" style="background: #ba9d0f !important;color: #ffffff;" class="btn btn-danger" onclick="guardarAlarma(${localStorage.getItem("idHabitacion")})">Activar Alarma <i class="fas fa-bell"></i></a>
        </div>
    </div>
    <!--/.Content-->
    </div>
...
async function guardarAlarma(params) {
    let hora=document.getElementById("tiempoAlarma").value
    hora=hora.split('T')
    hora=hora[0]+' '+hora[1]
    await fetch('moduloHabitaciones/php/addAlarma.php')
  .then(response => response.json())
