...
if (isset($_GET['combo'])) {//SI DETECTA QUE EL PRODUCTO ES UN COMBO AÑADE AL CARRITO Y DESCUENTA EL COMBO DE PRODUCTOS
	...
	$sqlArtiCom="SELECT * FROM `articulos` WHERE `articulo`=:id";
	$comboArti=$conexion->prepare($sqlArtiCom);
	$comboArti->bindParam(":id",$articulo);
	$comboArti->execute();
	$comboArti=$comboArti->fetch(PDO::FETCH_ASSOC);
