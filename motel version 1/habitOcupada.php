...
$selec="SELECT `habitacion`, `dirip`, `letra`, `nombre`, `descripcion`, `costo`, `ip_tablet`, `estado`,modoPrueba FROM `habitaciones` WHERE habitacion=:id";
...
$movtemSelect="SELECT `id`, `id2`, `codigo`, `habitacion`, `fechaActS1`, `activarDormida`, activarEstadia, `horaActS1`, `horaActS2`, `horaDesActS2`, `horaActS3`, `horaDesActS3`, `fechaDesActS1`, `horaDesActS1`, `S1`, `S2`, `S3` FROM `movtemp` WHERE habitacion=:id and id2=''";
...
<div style="border-radius: 10px;background: #0066bd91;box-shadow: 0px 5px 30px 10px #0000007a;" class="container my-5 p-5  animated pulse">
  ...
  <div style="width:40%;float:left;">
   ...
   <h3 style="border-radius:6px;background:#1dff1d99;color:white;">total: <?php $totalHABITACION=$habitacionjj+$contarCart['total']; echo "$".round($totalHABITACION)?></h3>
  ...
  <div style="width:40%;float:right;display:contents;">
        ...
        <!-- SI EL MODO PRUEBA ES == 1 ENTONCES HABILITO EL BOTON MODO PRUEBA -->
        <?php if($habitacion['modoPrueba']==1) {?>

          <h3><u>Herramienta prueba de arduino</u></h3>
          
          <a class="btn btn-success" onclick="disponible('<?php echo $habitacion['dirip']; ?>')"><i class="fas fa-times fa-2x"></i><b style="color:black;">Disponible</b></a>
          <a class="btn btn-blue" onclick="limpieza('<?php echo $habitacion['dirip']; ?>')"><i  class="fas fa-times fa-2x"></i><b style="color:black;">Limpieza</b></a>
          <a class="btn btn-danger" onclick="ocupada('<?php echo $habitacion['dirip']; ?>')"><i class="fas fa-times fa-2x"></i><b style="color:black;">Ocupada</b></a>
          <a class="btn" href="falloDeSensor.php?idMovtemp=<?php echo $movtem['id']."&habitacion=".$_GET['id']."&dirip=".$habitacion['dirip']?>" style="background:#948d8d;"><i style="margin-top:10px;color:black;" class="fas fa-times fa-2x"></i><b style="color:black;">Finalizar prueba</b></a>



          <!-- EN CASO CONTRARIO MUESTRO TODOS LOS OTROS BOTONES DE UNA SESION NORMAL -->
        <?php }else{ ?>
          <?php
          ...
          <a href="cambiarEstado.php?estado=ocupada&habitacion=<?php echo $habitacion['ip_tablet']."&idMovtemp=".$movtem['id']?>" class="btn btn-success " style="color:black;"><i style="margin-top:10px;"  class="fas fa-user fa-2x"></i>Continua</a>
            <?php if($productosPendientes['productosPendientes']<=0){?>
                <a href="metodosDePago.php?fin=fin&id=<?php echo $habitacion["habitacion"]."&id_movtem=".$movtem['id']."&ip_tablet=".$habitacion['ip_tablet']."&ipArduino=".$habitacion['dirip']."&totalCuenta=".round($totalHABITACION)?>" class="btn btn-danger" style="background-color:#533992;">Fin de pago</a>
            <?php }else{?>
          ...
          <a href="cambiarEstado.php?id=<?php echo $_GET['id'];?>" class="btn btn-yellow"><i style="margin-top:10px;color:black;" class="fas fa-dollar-sign fa-2x"></i><b style="color:black;">Enviar cuenta</b></a>
              ...
              <!-- ACTIVAR O DESACTIVAR DORMIDA -->
              <?php if($movtem['activarDormida']=="SI"){ ?>
                <a href="desactivarDormida.php?comando=dor&id=<?php echo $movtem['id']."&habit=".$_GET['id'];?>" class="btn" style="background:#722f2fd9;"><i style="margin-top:10px;color:black;" class="fas fa-dollar-sign fa-2x"></i><b style="color:black;">Desactivar dormida</b></a>
              <?php }else{ ?>
              ...
              <!-- ACTIVAR O DESACTIVAR ESTADIA -->
              <?php if($movtem['activarEstadia']=="SI"){ ?>
                <a href="desactivarDormida.php?comando=esta&id=<?php echo $movtem['id']."&habit=".$_GET['id'];?>" class="btn" style="background:#722f2fd9;"><i style="margin-top:10px;color:black;" class="fas fa-dollar-sign fa-2x"></i><b style="color:black;">Desactivar Estadia</b></a>
                <?php }else{ ?>
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div style="background:#dee2e6;" class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <a href="moduloTicket/index.php?ip=<?php echo $habitacion['ip_tablet']."&habitacion=".$habitacion['nombre']."&movtemp=".$movtem['id']."&idHabitacion=".$_GET['id']?>&comando=articulosCompletos&totalHabit=<?php echo round($habitacionjj);?>" class="btn btn-success">ticket</a>
      </div>
    </div>
  </div>
</div>
...
<div class="modal fade" id="modalHerramienta" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
  aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div style="background:#dee2e6;" class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Herramienta de arduino</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
